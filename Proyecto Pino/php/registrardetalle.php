<?php
        if($_SERVER['REQUEST_METHOD'] == 'POST')
        {
            ...
            $query = 'INSERT INTO detalles (titulo, descripcion, fecha, precio, cliente, vehiculo) VALUES ("'.$titulo.'","'.$desc.'",'.$fecha.','. $precio .','.$persona[0][0].','.$vehiculo[0][0].')';
            $result = mysqli_query($conn,$query);
            if(is_bool($result))
            {
                if($result == true)
                ...
                {
                    echo 'Oops! Algo ha salido mal al registrar el detalle. ponte en contacto con un supervisor.';
                    echo '<script>console.error("'.mysqli_error($conn).'");'.'console.log("'.$query.'");'.'</script>';
                    exit();
                }
            }
